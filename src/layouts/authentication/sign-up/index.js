...
function InvoicePreview() {
  ...
  const handleUpdateProduct = async (oldDesc, newStockCode) => {
    try {
      const response = await fetch("http://localhost:8000/api/update-invoice-product/", {
        method: "POST",
        headers: {
        ...
        {ambiguousProducts.map((item, index) => (
          <MDBox key={index} sx={{ mb: 3 }}>
            ...
            <Select
              fullWidth
              defaultValue=""
              onChange={(e) => handleUpdateProduct(item.Description, e.target.value)}
            >
